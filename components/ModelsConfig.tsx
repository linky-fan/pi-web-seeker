...
const COST_LABEL_KEYS: Record<keyof NonNullable<ModelEntry["cost"]>, string> = {
  input: "stats.input",
  output: "stats.output",
...
function ModelDetail({
  ...
  const testSummary = (() => {
    ...
    if (testState.phase === "success") {
      return [t("models.connectedSummary"), ...meta, testState.responseText || null].filter(Boolean).join(" · ");
    }
    return [t("models.failedSummary"), ...meta, testState.message].filter(Boolean).join(" · ");
  })();
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      const res = await fetch(apiPath("models-config/test"), {
        method: "POST",
...
function OAuthDetail({ provider, onRefresh }: { provider: OAuthProvider; onRefresh: () => void }) {
  ...
  const handleLogin = useCallback(() => {
    ...
    const es = new EventSource(apiPath(`auth/login/${encodeURIComponent(provider.id)}`));
  ...
  const handleLogout = useCallback(async () => {
    await fetch(apiPath(`auth/logout/${encodeURIComponent(provider.id)}`), { method: "POST" });
    setLoginState({ phase: "idle" });
    onRefresh();
  ...
  const submitCode = useCallback(async (token: string, code: string) => {
    ...
    try {
      const res = await fetch(apiPath(`auth/login/${encodeURIComponent(provider.id)}`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        setLoginState({ phase: "error", message: d.error ?? t("models.serverError", { status: res.status }) });
        return;
      }
      setInputValue("");
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: provider.loggedIn ? "#4ade80" : "var(--text-dim)" }}>
            {provider.loggedIn ? t("models.connected") : t("models.notConnected")}
          </span>
        </div>
      </div>
      ...
      <div style={{ minHeight: 48 }}>
        {loginState.phase === "idle" && (
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
            {provider.loggedIn ? t("models.alreadyConnected") : t("models.connectAccount", { provider: provider.name })}
          </p>
        )}
...
function ApiKeyDetail({ provider, onRefresh }: { provider: ApiKeyProvider; onRefresh: () => void }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setSavedOk(false);
    try {
      const res = await fetch(apiPath(`auth/api-key/${encodeURIComponent(provider.id)}`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleRemove = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(apiPath(`auth/api-key/${encodeURIComponent(provider.id)}`), { method: "DELETE" });
      const d = await res.json() as { success?: boolean; error?: string };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {provider.configured
          ...
          : t("models.enterApiKey", { provider: provider.displayName, models: modelCountText(t, provider.modelCount) })}
      </p>
              ...
              {availableApiKey.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{modelCountText(t, p.modelCount)}</div>
                  </div>
                  <ProviderIcon id={p.id} size={28} />
                </button>
              ))}
...
export function ModelsConfig({ onClose }: { onClose: () => void }) {
  ...
  const loadApiKeyProviders = useCallback(() => {
    fetch(apiPath("auth/all-providers"))
      .then((r) => r.json())
  ...
  const handleSave = useCallback(async () => {
    ...
    setSavedOk(false);
    try {
      const res = await fetch(apiPath("models-config"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
